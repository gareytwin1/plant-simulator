...
4. **Check the model assignment.** `BUILD_PLAN_STATUS.json` does not carry
   one — the assignment lives in the `AGENT` map in `docs/BUILD_PLAN.html`
   (grep the task ID), which defaults to Opus for a `core` task and Sonnet for
   everything else. PROJECT_STATE.md's startable-tasks table repeats it for
   the tasks listed there. Cross-check the result against
