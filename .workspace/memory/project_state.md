...
**Last state refresh:** 25 September 2026, at `df1f91f` (Merge T8-1: PID
block, PR #76) - **this is a snapshot, not a live pointer.** Run
`git log df1f91f..HEAD --oneline` to see what has merged since.
...
**In flight:** nothing. **No spine lock is
...
| **M8** PID Controllers and Modes | 1/5 - T8-1 Complete; T8-2 now startable |
...
**57 of 114 tasks Complete.** Next checkpoint is **C** (M5 + M6 + M7); M5 is
...
| **T8-2** | Control modes and bumpless transfer | Sonnet | `feature/control-modes` |
...
**Still waiting:** T8-3 and T9-2 — on T8-2 (now startable, not yet complete)
and T9-1. T11-1 still waits on T9-1 too.
...
spine queue is empty since T6-5. **T18-1 must run exactly one Gunicorn
...
satellite work under the `app/engine/` rule in AGENTS.md. T8-2 depends on
T8-1's `PID` class in `app/controls/pid.py`.
